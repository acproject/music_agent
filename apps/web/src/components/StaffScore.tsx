...
function toTickable(item: ScoreItem): StaveNote {
...
export default function StaffScore({ score }: { score: QuantizedScore | null }) {
  ...
  useEffect(() => {
    ...
    score.measures.forEach((measure, i) => {
      ...
      voice.addTickables(measure.items.map(toTickable));
