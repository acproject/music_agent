...
- **M2** 实时链路：YIN 音高检测、WebSocket 透传、前端麦克风采集与音高曲线；
  采集同时在本地分块累积 16kHz Float32 PCM（暂停不录、断线不丢），停止时一次性
  合并写入共享录音上下文，无需转谱即可直接送 AI 老师工具分析
...
- **M8** 结构化训练面板（视唱训练 / 节奏训练）：全链路离线规则化、不依赖 LLM——
  `domain/exercise.ts` 按难度（初/中/高级）在约束下随机生成 4 小节 4/4、C 大调练习
  （时值白名单保证一次攻击一个谱条目、终止式强制解决到主音、节奏练习统一音高且
  targets 直接构建不被相邻同音合并），支持「换一条」新种子；
  `domain/measureFeedback.ts` 把 PerformanceReport 按小节聚合，独立重算每小节
  音准/节奏/完整度加权分，多音经 tempo map 折算小节，并生成规则文案
  （漏唱/漏拍、唱成X应为Y、抢拍/拖后 ms、偏高/偏低音分、多音）；
  共享 `useTrainingSession` hook 编排「授权 → 2 拍预备（预备拍帧丢弃）→ 录音 →
  notes 检测 → comparePerformance → 小节反馈」，`StaffScore` 支持逐音着色
  （ok 绿/抢拖拍琥珀/错音红），报告视图含总评指标条与逐小节卡片；
  58 个测试，其中练习生成器经 30 种子 × 两类型 × 三难度宽扫描验证不变量
